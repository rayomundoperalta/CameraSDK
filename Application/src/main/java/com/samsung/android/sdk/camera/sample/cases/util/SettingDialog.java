...
public class SettingDialog implements SettingItem.OnSettingItemValueChangedListener {
    ...
    public void configure(SCameraCharacteristics characteristics, int lensFacing, List<Integer> availableLensFacing,
                          int imageFormat, Size jpegSize, List<Integer> availableImageFormat, SCaptureRequest.Builder... builders) {
        mConfigured = false;
        mDimMap.clear();
        ...
        for (SCaptureRequest.Builder builder : builders) {
            ...
            if (builder.get(SCaptureRequest.CONTROL_CAPTURE_INTENT) == SCaptureRequest.CONTROL_CAPTURE_INTENT_PREVIEW) {
                for (SettingItem<?, ?> item : mSettingItemMap.values()) {
                    ...
                    } else if (item.getKey() instanceof Integer && (Integer) item.getKey() == SettingItem.SETTING_TYPE_JPEG_SIZE) {
                        item.setInitialValue(jpegSize);
                    }
                }
            }
        }
    ...
    public <K, V> void onSettingItemValueChanged(K key, V value) {
        ...
        if (key == SCaptureRequest.CONTROL_AF_MODE) {
            ...
            switch ((Integer) value) {
                case ImageFormat.JPEG:
                    ...
                    setDim(false, SettingItem.SETTING_TYPE_JPEG_SIZE);
                    break;
                case ImageFormat.RAW_SENSOR:
                    ...
                    setDim(true, SettingItem.SETTING_TYPE_JPEG_SIZE);
                    break;
            }
        } else if (key instanceof Integer && (Integer) key == SettingItem.SETTING_TYPE_CAMERA_FACING) {

        } else if (key == SCaptureRequest.EDGE_MODE) {

        } else if (key == SCaptureRequest.NOISE_REDUCTION_MODE) {
        ...
        if (mBuilders != null) {
            for (SCaptureRequest.Builder builder : mBuilders) {
                if (key instanceof SCaptureRequest.Key) {
                    ...
                    } else if ((Integer) key == SettingItem.SETTING_TYPE_JPEG_SIZE) {
        ...
        if (mListener != null && mConfigured) {
            ...
            else mListener.onCameraSettingUpdated((Integer) key, value);
        }
    }
     ...
     */
    private <V> View addSettingItem(String name, Integer key, List<String> valueNameList, List<V> valueList, List<V> availableValueList) {
        ListSettingItem<Integer, V> settingItem = new ListSettingItem<Integer, V>(mActivity, name, key, valueNameList, valueList, availableValueList);
    ...
    private void initDialog(SCameraCharacteristics characteristics, List<Integer> availableLensFacing, List<Integer> availableImageFormat) {
        ...
        // jpeg size
        {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && characteristics.get(SCameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP).getHighResolutionOutputSizes(ImageFormat.JPEG) != null) {
                jpegSizeList.addAll(Arrays.asList(characteristics.get(SCameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP).getHighResolutionOutputSizes(ImageFormat.JPEG)));
            }
            jpegSizeList.addAll(Arrays.asList(characteristics.get(SCameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP).getOutputSizes(ImageFormat.JPEG)));
            List<String> jpegSizeStringList = new ArrayList<String>();

            for (Size size : jpegSizeList) jpegSizeStringList.add(size.toString());

            ((LinearLayout) dialogView.findViewById(R.id.settinglist)).addView(
                    addSettingItem("Jpeg size",
                            Integer.valueOf(SettingItem.SETTING_TYPE_JPEG_SIZE),
                            jpegSizeStringList,
                            jpegSizeList,
                            jpegSizeList),
                    layoutParams);
        ...
        {
            if (characteristics.getKeys().contains(SCameraCharacteristics.NOISE_REDUCTION_AVAILABLE_NOISE_REDUCTION_MODES)) {
                ((LinearLayout) dialogView.findViewById(R.id.settinglist)).addView(
                        addSettingItem("Noise reduction mode",
                                SCaptureRequest.NOISE_REDUCTION_MODE,
                                Arrays.asList("Off", "Fast", "High Quality", "Minimal", "ZSL"),
                                Arrays.asList(SCaptureRequest.NOISE_REDUCTION_MODE_OFF,
                                        SCaptureRequest.NOISE_REDUCTION_MODE_FAST,
                                        SCaptureRequest.NOISE_REDUCTION_MODE_HIGH_QUALITY,
                                        SCaptureRequest.NOISE_REDUCTION_MODE_MINIMAL,
                                        SCaptureRequest.NOISE_REDUCTION_MODE_ZERO_SHUTTER_LAG),
                                asList(characteristics.get(SCameraCharacteristics.NOISE_REDUCTION_AVAILABLE_NOISE_REDUCTION_MODES))),
                        layoutParams);
            } else {
                ((LinearLayout) dialogView.findViewById(R.id.settinglist)).addView(addSettingItem("Noise reduction mode"), layoutParams);
            }
        }

        // edge mode
        {
            if (characteristics.getKeys().contains(SCameraCharacteristics.EDGE_AVAILABLE_EDGE_MODES)) {
                ((LinearLayout) dialogView.findViewById(R.id.settinglist)).addView(
                        addSettingItem("Edge mode",
                                SCaptureRequest.EDGE_MODE,
                                Arrays.asList("Off", "Fast", "High Quality", "ZSL"),
                                Arrays.asList(SCaptureRequest.EDGE_MODE_OFF,
                                        SCaptureRequest.EDGE_MODE_FAST,
                                        SCaptureRequest.EDGE_MODE_HIGH_QUALITY,
                                        SCaptureRequest.EDGE_MODE_ZERO_SHUTTER_LAG),
                                asList(characteristics.get(SCameraCharacteristics.EDGE_AVAILABLE_EDGE_MODES))),
                        layoutParams);
            } else {
                ((LinearLayout) dialogView.findViewById(R.id.settinglist)).addView(addSettingItem("Edge mode"), layoutParams);
            }
        }
    ...
    /**
     * control dim state for int type setting item.
     */
    private <T> void setDim(boolean isDim, int key) {
        if (!mSettingItemMap.containsKey(key)) return;
    ...
    public interface OnCameraSettingUpdatedListener {
        public void onCameraSettingUpdated(int key, Object value);
    }
}
